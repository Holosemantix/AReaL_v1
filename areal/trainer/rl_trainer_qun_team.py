...
class PPOTrainer:
            ...
            if isinstance(self.valid_dataloader, dict):
                    ...
                    for data in dataloader:
                        for item in data:
                            self.eval_rollout.submit(
                                item,
                                eval_workflow,
                                eval_workflow_kwargs[dataset_name],
                                group_size=self.config.eval_gconfig.n_samples,
                                is_eval=True,
                            )
                            cnt += 1
                    logger.info(f"Submitted {cnt} samples evaluated, dp_rank={self.actor.data_parallel_rank}, dataset: '{dataset_name}'.")
                ...
                for data in self.valid_dataloader:
                    for item in data:
                        ...
                        cnt += 1
                logger.info(f"Submitted {cnt} samples evaluated.")
                self.eval_rollout.wait(cnt, timeout=None)
    ...
    def _export_and_commit_stats(self, epoch: int, epoch_step: int, global_step: int):
        ...
        self._save_log_metrics(metrics=stats, epoch=epoch, epoch_step=epoch_step, global_step=global_step)

        dist.barrier(group=self.actor.cpu_group)
        current_platform.synchronize()

    def _save_log_metrics(self, metrics: dict, epoch: int, epoch_step: int, global_step: int):
        if int(os.getenv("RANK", "0")) == 0:
            while True:
                try:
                    save_path = os.path.join(
                        self.config.cluster.fileroot, "logs/root", self.config.experiment_name, self.config.trial_name,
                        f"{self.config.experiment_name}_{self.config.trial_name}_logger_metrics.jsonl")
