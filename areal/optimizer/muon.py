    ...
    def step(self):
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                # If ns_input is a DTensor, it represents the fully sharded FSDP state
                is_dtensor = HAS_DTENSOR and isinstance(ns_input, DTensor)

                if is_dtensor:
                    # Gather the full, un-sharded tensor for 2D Matrix calculations
                    ns_input_full = ns_input.full_tensor()
                else:
                    ns_input_full = ns_input

                # gather tensor parallel ( if tp )
                if tp_split_dim != -1:
                    ns_input_shards = [torch.empty_like(ns_input_full) for _ in range(tp_world_size)]
                    dist.all_gather(ns_input_shards, ns_input_full, self.tp_group)
                    ns_input_full = torch.cat(ns_input_shards, dim=tp_split_dim)

                # calc update
                update = zeropower_via_newtonschulz5(ns_input_full, steps=ns_steps)

                # only local tp part
                if tp_split_dim != -1:
                    update = update.chunk(tp_world_size, dim=tp_split_dim)[tp_rank]

                # ====== FSDP2 DTensor Sharding ======
                if is_dtensor:
                    # Shard the full update back to a DTensor matching the parameter's placements.
                    # This safely and locally slices the computed global update to FSDP shards.
                    update = distribute_tensor(update, ns_input.device_mesh, ns_input.placements)
