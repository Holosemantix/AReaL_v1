...
def launch_sglang_server(argv):
    ...
    if config.sglang.model_path and os.path.isdir(config.sglang.model_path):
        import json
        import shutil
        import tempfile

        tmp_dir = tempfile.mkdtemp(prefix="sglang_model_")
        for name in os.listdir(config.sglang.model_path):
            src = os.path.join(config.sglang.model_path, name)
        ...
        config.sglang.model_path = tmp_dir
